...
proc wapp-safety-check {} {
    ...
    #                    other than $fromip
    #
    proc wappInt-start-listener {port callback} {
	set server [list wappInt-new-connection $callback]
    ...
    #
    proc wappInt-new-connection {callback wappmode callback chan ip port} {
	...
	fileevent $chan readable [list wappInt-http-readable $W $callback $chan]
    }
    ...
    #
    proc wappInt-http-readable {W callback chan} {
	if {[catch [list wappInt-http-readable-unsafe $W $callback $chan] msg]} {
	    puts stderr "$msg\n$::errorInfo"
	    wappInt-close-channel $chan
	}
    }
    proc wappInt-http-readable-unsafe {W callback chan} {
	if {![dict exists $W .toread]} {
		...
		if {[wappInt-parse-header $W $chan]} {
		    catch {close $chan}
		    return
		}
		set len 0
		...
		wappInt-handle-request $W $callback $chan 0
	    }
	}
	fileevent $chan readable [list wappInt-http-readable $W $callback $chan]
    }
    ...
    #
    proc wappInt-parse-header {W chan} {
    ...
    proc wapp-start {port callback} {
	wappInt-start-listener $port $callback
    }
